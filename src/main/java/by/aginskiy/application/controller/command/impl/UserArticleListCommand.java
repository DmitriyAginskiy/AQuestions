...
/**
 * Action command to display the user articles list.
 *
 * @author Dzmitry Ahinski
 */
public class UserArticleListCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        try {
            ...
            logger.log(Level.ERROR, "Error while trying to get user article list or user articles number, ",
                    exception);
            throw new CommandException(exception);
        }
        return new CommandResult(JspPath.INFO_PAGE, CommandResult.TransitionType.FORWARD);
    }
}
