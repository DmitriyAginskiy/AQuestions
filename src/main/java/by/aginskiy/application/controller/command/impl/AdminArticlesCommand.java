...
/**
 * Action command to display the articles list to the administrator.
 *
 * @author Dzmitry Ahinski
 */
public class AdminArticlesCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        try {
            List<Article> articles = GlobalArticleServiceImpl.INSTANCE.findAllActive(start, limit);
            request.setAttribute(RequestParameter.CURRENT_ENTITY_LIST, articles);
            int articlesNumber = GlobalArticleServiceImpl.INSTANCE.calculateArticles();
            ...
            request.setAttribute(RequestParameter.COMMAND, CommandType.ADMIN_ARTICLES.name());
        } catch (ServiceException exception) {
            logger.log(Level.ERROR, "Error while trying to find articles or calculate their number, ",
                    exception);
