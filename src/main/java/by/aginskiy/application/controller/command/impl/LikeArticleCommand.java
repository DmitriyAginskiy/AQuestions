...
/**
 * Action command to like the article by the user.
 *
 * @author Dzmitry Ahinski
 */
public class LikeArticleCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        try {
            if(RatingServiceImpl.INSTANCE.isRatedByUser(user.getId(), articleId)) {
                ...
                RatingServiceImpl.INSTANCE.addRatingByUser(user.getId(), articleId, Rating.LIKED);
            }
            commandResult = new CommandResult(UrlPath.VIEW_QUESTION_DO, CommandResult.TransitionType.REDIRECT);
        } catch (ServiceException exception) {
            logger.log(Level.ERROR, "Error while trying update or to add rating by user, ", exception);
