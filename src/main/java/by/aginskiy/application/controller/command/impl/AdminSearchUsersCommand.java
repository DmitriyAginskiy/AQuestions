...
/**
 * Action command to search for users by criteria by the administrator.
 *
 * @author Dzmitry Ahinski
 */
public class AdminSearchUsersCommand implements Command {
    ...
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        try {
            if(SearchValidator.isValidSearchText(criteria)) {
                List<User> users = UserServiceImpl.INSTANCE.findAllByCriteria(criteria, start, limit);
                request.setAttribute(RequestParameter.CURRENT_ENTITY_LIST, users);
                int usersNumber = UserServiceImpl.INSTANCE.calculateUsersByCriteria(criteria);
                ...
                request.setAttribute(RequestParameter.COMMAND, CommandType.ADMIN_SEARCH_USERS.name());
            ...
            logger.log(Level.ERROR, "Error while trying to find users by criteria or to calculate their number, ",
                    exception);
