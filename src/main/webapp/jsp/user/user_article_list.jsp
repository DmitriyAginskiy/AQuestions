...
<div class="container-lg mt-1 mx-auto mb-4" style="background-color: #26292E; padding: 25px; border-radius: 10px;">
    ...
    <c:choose>
                        ...
                        <div class="w-100" style="margin: auto 0;">
                            ...
                            <div class="w-100 d-flex justify-content-between align-items-center">
                                ...
                                <div class="d-flex w-50">
                                    <c:choose>
                                        <c:when test="${article.blocked eq true}">
                                            <h2 class="fs-2 mx-auto" style="color: #DE5E60;"><fmt:message key="info.blocked" /></h2>
                                        </c:when>
                                        <c:otherwise>
                                            <a href="view_article.do?article_id=${article.id}" class="btn btn-primary me-1" style="background-color: #DE5E60; border: none; width: 33%;"><fmt:message key="info.view" /></a>
                                            <c:choose>
                                                <c:when test="${article.active eq true}">
                                                    <a href="hide_article.do?article_id=${article.id}" class="btn btn-primary me-1" style="background-color: #DE5E60; border: none; width: 33%;"><fmt:message key="info.hide" /></a>
                                                </c:when>
                                                <c:otherwise>
                                                    <a href="recover_article.do?article_id=${article.id}" class="btn btn-primary me-1" style="background-color: #DE5E60; border: none; width: 33%;"><fmt:message key="info.show" /></a>
                                                </c:otherwise>
                                            </c:choose>
                                            <a href="to_edit_article.do?article_id=${article.id}" class="btn btn-primary me-1" style="background-color: #DE5E60; border: none; width: 33%;"><fmt:message key="info.edit_button" /></a>
                                        </c:otherwise>
                                    </c:choose>
                                </div>
                            </div>
        ...
        <c:otherwise>
            <h2 class="d-flex justify-content-center mt-3 mb-5 fs-5" style="color: #DE5E60;"><fmt:message key="info.questions_list" /></h2>
        </c:otherwise>
    </c:choose>
</div>
<ctg:pagination command="USER_ARTICLE_LIST" />
